...
for (let i = 0; i < people.length; i++) {
  ...
  people[i].cities = Math.floor(Math.random() * cities.lenght);
...
function diceRoller(number, side = 6) {
  const resultArray = [];
...
function cleanUp(array) {
  ...
  for (const word of array) {
    cleanArray.push(word.trim().toLowerCase());
  }
  return cleanArray.join(" ");
}

console.log(
  cleanUp([" thIS", "teXt  ", " nEeds ", "to", "BE", "cleANED   ", " Up"])
);
